...
- **`trizod/potenci/`** — Predicts random coil chemical shifts using the [POTENCI](https://github.com/protein-nmr/POTENCI) algorithm (see [potenci/README.md](trizod/potenci/README.md))
- **`trizod/scoring/`** — Computes per-residue CheZOD Z-scores and TriZOD G-scores from experimental vs. predicted shifts

The main pipeline (`trizod/trizod.py`) orchestrates these modules: parse BMRB entries, predict random coil shifts, compute disorder scores, and apply configurable quality filters.
