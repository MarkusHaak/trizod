...
class OffsetTooLargeError(Exception):
    pass


class OffsetCausedFilterError(Exception):
    pass
            ...
            if os.path.exists(cache_fp):
                try:
                    ...
                    logging.getLogger("trizod.bmrb").debug(
                        f"cache file {cache_fp} corrupt or formatted wrong: {e}"
                    )
...
def prefilter_dataframe(
    ...
    method_whitelist_ = [method.lower() for method in method_whitelist]
    if method_whitelist_:
        ...
        method_sel = False
    method_blacklist_ = [method.lower() for method in method_blacklist]
    ...
    sels_pre = {
        ...
        ("temperature", f"{list(temperature_range)}"): (df.temperature >= temperature_range[0])
        ...
        try:
            for sID in sampleIDs:
                for comp in entry.samples[sID].components:
                    if (
                        comp[3] and not comp[2] and den_comp.lower() in comp[3].lower()
                    ):  # if it has a name but no entity entry
                        row[den_comp] = True
                        raise FoundError
        except FoundError:
            pass
