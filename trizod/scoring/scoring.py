...
def comp2pred_arr(predshiftdct, bbshifts_arr, bbshifts_mask):
    ...
    for res, aa in predshiftdct:
        ...
        for j, at in enumerate(BBATNS):
            if at in predshiftdct[(res, aa)] and predshiftdct[(res, aa)][at] is not None:
...
def compute_zscores(ashwi3, k3, mask, corr=False):
    ...
    totn3f_ = k3
    cdfs3_ = conv_chi2_cdf(tot3f_, totn3f_)
    if corr:
...
def compute_pscores(ashwi3, k3, mask, quotient=2.0, limit=4.0):
    ...
    if limit:
        p = np.prod(
            scipy.stats.norm.pdf(np.minimum(ashwi3, limit) / quotient) / scipy.stats.norm.pdf(0.0),
            axis=1,
        )
        ...
        p = np.prod(scipy.stats.norm.pdf(ashwi3 / quotient) / scipy.stats.norm.pdf(0.0), axis=1)
        with np.errstate(divide="ignore"):  # zeros are to be expected; resulting NANs are ok
            p = p ** (1 / k3)
    p[k3 == 0] = np.nan
    p[:mini] = np.nan
...
def get_offset_corrected_wscs(
    ...
    if offr is None:
        logging.getLogger("trizod.scoring").warning("no running offset could be estimated")
