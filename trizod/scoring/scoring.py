...
def comp2pred_arr(predshiftdct, bbshifts_arr, bbshifts_mask):
    ...
    for res, aa in predshiftdct:
        ...
        for j, at in enumerate(BBATNS):
            if (
                at in predshiftdct[(res, aa)]
                and predshiftdct[(res, aa)][at] is not None
            ):
...
def compute_running_offsets(cmparr, mask, minAIC=999.0):
    ...
    for i in range(
        7
    ):  # TODO: not necessary to compute anything but at_stdc for all values. Only the selected position would suffice
    ...
    runstds_val = (
        runstds_[runstds_.columns[mask.any(axis=0)]].dropna(axis=0).mean(axis=1)
    )
    try:
    ...
    offdct_ = {}
    for col in runstds_.dropna(
        how="all", axis=1
    ).columns:  # for all at shifts that were detected anywhere in this sample
        at = BBATNS[col]
        roff = runoffs_.loc[min_idx_][col]
        std0 = runstd0s_.loc[min_idx_][col]
        stdc = runstds_.loc[min_idx_][col]
        dAIC = (
            np.log(std0 / stdc) * 9 - 1
        )  # difference in Akaike’s information criterion, 9 is width of window
        logging.getLogger("trizod.scoring").info(
            f"minimum running average: {at} {roff} {dAIC}"
        )
        if dAIC > minAIC:
            logging.getLogger("trizod.scoring").info(
                f"using offset correction: {at} {roff} {dAIC}"
            )
            offdct_[at] = roff
        else:
            logging.getLogger("trizod.scoring").info(
                f"rejecting offset correction due to low dAIC: {at} {roff} {dAIC}"
            )
            # offdct_[at] = 0.0
...
def compute_offsets(shw_, accdct_, minAIC=999.0):
    ...
    newoffdct_[reject_mask] = 0.0
    newoffdct_ = dict(zip(BBATNS, newoffdct_))
    return newoffdct_
...
def get_outlier_mask(cdfs3_, cdfs_, ashwi_, mask, cdfthr=6.0):
    ...
    ol_ = mask & (
        np.bitwise_or(np.expand_dims(finaloutli_, axis=1), oldct_)
    )  # mask for the outliers
    return ol_


def get_std_norm_diffs(cmparr, mask, offdct=None):
    if offdct is None:
        offdct = {}
    w_ = np.array([REFINED_WEIGHTS[at] for at in BBATNS])  # ensure same order
    off_ = np.array([offdct.get(at, 0.0) for at in BBATNS])
    shw_ = cmparr / w_
...
def compute_pscores(ashwi3, k3, mask, quotient=2.0, limit=4.0):
    ...
    if limit:
        p = np.prod(
            scipy.stats.norm.pdf(np.minimum(ashwi3, limit) / quotient)
        ...
        p = np.prod(
            scipy.stats.norm.pdf(ashwi3 / quotient) / scipy.stats.norm.pdf(0.0), axis=1
        )
        with np.errstate(
            divide="ignore"
        ):  # zeros are to be expected; resulting NANs are ok
...
def convert_to_triplet_data(ashwi_, mask):
    ashwi3 = ashwi_.copy()
    ashwi3[~mask] = 0.0
    ashwi3 = np.column_stack(
        [
            np.pad(ashwi3, ((1, 1), (0, 0)))[2:],
            ashwi3,
            np.pad(ashwi3, ((1, 1), (0, 0)))[:-2],
        ]
    )
...
def get_offset_corrected_wSCS(seq, shifts, predshiftdct):
    ...
    if totbbsh == 0:
        ...
        return
    logging.getLogger("trizod.scoring").info(
        f"total number of backbone shifts: {totbbsh}"
    )
    ...
    if offr is None:
    ...
    elif np.any([v != 0.0 for v in offr.values()]):
        ...
        if (
            av0 >= avc
        ):  # use offset correction only if it leads to, in average, better accordance with the POTENCI model (more disordered)
            olc = get_outlier_mask(cdfs3c, cdfsc, ashwic, cmp_mask, cdfthr=6.0)
            noffc = compute_offsets(shwc, cmp_mask & ~olc, minAIC=6.0)
            offf = noffc
            olf = olc
